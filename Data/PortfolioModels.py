...
class MarkovModel:
    ...
    def result(self) -> list[str]:
        ...
        minimal_risk = DataFrame([portf[min_risk] * 100], columns=self.df_close.columns, index=['part, %']).T
        minimal_risk['Price now'] = self.get_last_prices()
        minimal_risk['Number of shares'] = ((minimal_risk['part, %'] / 100) * self.portfolio_cost) / minimal_risk['Price now']
        minimal_risk['Number of shares'] = minimal_risk['Number of shares'].round(0)
        minimal_risk = minimal_risk.round(2)
        del minimal_risk[minimal_risk.columns[1]]

        maximum_sharpe_ratio = DataFrame([portf[max_sharp_coefficient] * 100], columns=self.df_close.columns, index=['part, %']).T
        maximum_sharpe_ratio['Price now'] = self.get_last_prices()
        maximum_sharpe_ratio['Number of shares'] = ((maximum_sharpe_ratio['part, %'] / 100) * self.portfolio_cost) / maximum_sharpe_ratio['Price now']
        maximum_sharpe_ratio['Number of shares'] = maximum_sharpe_ratio['Number of shares'].round(0)
        maximum_sharpe_ratio = maximum_sharpe_ratio.round(2)
        del maximum_sharpe_ratio[maximum_sharpe_ratio.columns[1]]

        medium_portfolio = DataFrame([r_mean * 100], columns=self.df_close.columns, index=['part, %']).T
        medium_portfolio['Price now'] = self.get_last_prices()
        medium_portfolio['Number of shares'] = ((medium_portfolio['part, %'] / 100) * self.portfolio_cost) / medium_portfolio['Price now']
        medium_portfolio['Number of shares'] = medium_portfolio['Number of shares'].round(0)
        medium_portfolio = medium_portfolio.round(2)
        del medium_portfolio[medium_portfolio.columns[1]]

        portfolio_structure.append('Min  risk\n')
        portfolio_structure.append("risk = %1.2f%%" % (float(risk[min_risk]) * 100.))
        ...
        portfolio_structure.append('\nAverage portfolio\n')
        portfolio_structure.append("risk = %1.2f%%" % (float(risk_mean) * 100.))
